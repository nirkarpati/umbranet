...
class EpisodicProcessor:
    ...
    def __init__(self):
        self.episodic_store: EpisodicMemoryStore | None = None
    ...
    async def process_job(self, job: MemoryReflectionJob) -> dict[str, Any]:
        ...
        try:
            ...
            if not curation_result.should_store:
                logger.info(
                    f"📝 Episodic curator decided not to store job {job.job_id}: "
                    f"{curation_result.reasoning}"
                )
            ...
            logger.info(
                f"📝 Episodic curator storing job {job.job_id} with importance "
                f"{curation_result.importance_score:.1f}"
            )
            ...
            async with self.episodic_store:
                ...
                # Add temporal grounding metadata if available
                if curation_result.occurred_at:
                    enhanced_metadata["occurred_at"] = curation_result.occurred_at
                
                # Use the temporal-aware log_episode method if occurred_at is available
                if curation_result.occurred_at:
                    # Parse the occurred_at timestamp
                    try:
                        occurred_timestamp = datetime.fromisoformat(
                            curation_result.occurred_at
                        )
                    except ValueError:
                        logger.warning(
                            f"Invalid occurred_at timestamp: "
                            f"{curation_result.occurred_at}"
                        )
                        occurred_timestamp = None
                    
                    episode_id = await self.episodic_store.log_episode(
                        user_id=job.user_id,
                        content=curation_result.content_to_store,
                        metadata=enhanced_metadata,
                        timestamp=occurred_timestamp
                    )
                else:
                    # Fallback to standard log_interaction for backwards compatibility
                ...
                logger.info(
                    f"✅ Episodic processing completed for job {job.job_id} "
            ...
            logger.error(
                f"❌ Episodic processing failed for job {job.job_id} "
