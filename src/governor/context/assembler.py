...
class ContextAssembler:
    ...
    async def _gather_context_data(
        ...
        try:
            async with self.procedural_memory as proc_store:
                ...
                persona_task = asyncio.create_task(self._get_persona(user_id, proc_store))
                environment_task = asyncio.create_task(self._get_environment_context(user_id, proc_store))
                memory_task = asyncio.create_task(self._get_memory_context(user_id, current_input, state))
                tasks_task = asyncio.create_task(self._get_active_tasks(user_id, state))
                instructions_task = asyncio.create_task(self._get_relevant_instructions(user_id, current_input, proc_store))
                
                # Wait for all context components
                persona, environment, memory, tasks, instructions = await asyncio.gather(
                    persona_task, environment_task, memory_task, tasks_task, instructions_task,
            ...
            try:
                memory = await self._get_memory_context(user_id, current_input, state)
                tasks = await self._get_active_tasks(user_id, state)
            except Exception:
                # If memory/tasks fail too, use empty defaults
                memory = {}
                tasks = []
    ...
    async def _get_persona(self, user_id: str, proc_store: ProceduralMemoryStore) -> str:
        ...
        try:
            ...
            if comm_style == "casual":
                ...
                customizations.append("- Use formal, professional language at all times")
            
            # Response format preferences
            format_pref = profile.get_value("preferences", "response_format", "balanced")
...
- Provide status updates for long-running operations"""
    ...
    async def _get_environment_context(self, user_id: str, proc_store: ProceduralMemoryStore) -> Dict[str, Any]:
    ...
    async def _get_memory_context(
        ...
        """
        # Extract recent conversation history from state
        recent_messages = []
        if hasattr(state, 'conversation_history') and state.conversation_history:
            # Get last 5 exchanges for context
            recent_messages = state.conversation_history[-10:]  # Last 10 messages
        
        return {
            "recent_conversation": recent_messages,
            "conversation_length": len(state.conversation_history) if hasattr(state, 'conversation_history') else 0,
    ...
    async def _get_active_tasks(self, user_id: str, state: GovernorState) -> List[Dict[str, Any]]:
        ...
        if state.pending_tools:
            for tool in state.pending_tools:
                tasks.append({
                    ...
                    "status": "awaiting_confirmation" if state.awaiting_confirmation else "pending",
    ...
    def _build_prompt(
        ...
        prompt_parts = [
            ...
            f"- Day: {context_data.environment['day_of_week']} ({context_data.environment['time_period']})",
        ...
        if context_data.memory.get('relevant_instructions'):
            ...
            for instr in context_data.memory['relevant_instructions']:
                prompt_parts.append(f"- {instr['title']}: {instr['instruction']}")
                prompt_parts.append(f"  (Category: {instr['category']}, Relevance: {instr['relevance']:.2f})")
            prompt_parts.append("")
        ...
        if context_data.memory.get('recent_conversation'):
            prompt_parts.extend([
                "RECENT CONVERSATION:",
                f"- Session length: {context_data.memory['session_age_minutes']} minutes",
                f"- Total exchanges: {context_data.memory['conversation_length']}",
                f"- Topics: {', '.join(context_data.memory['conversation_topics'])}",
                ""
            ])
        ...
        if context_data.tasks:
            ...
            for task in context_data.tasks:
                prompt_parts.append(f"- {task['type']}: {task.get('tool_name', 'N/A')} ({task['status']})")
            prompt_parts.append("")
        ...
        prompt_parts.extend([
            ...
            f"- Awaiting Confirmation: {'Yes' if state.awaiting_confirmation else 'No'}",
            ""
        ])
    ...
    def _get_user_preferences(self, user_id: str) -> Dict[str, Any]:
        ...
        """
        # This would integrate with user preference storage in production
        return {
    ...
    def _generate_session_summary(self, state: GovernorState) -> str:
        ...
        if turns == 0:
            ...
            return f"Productive session ({tools_executed} tools executed, {errors} errors)"
