...
class GraphMaintenanceTools:
    ...
    def __init__(
        self, neo4j_client: Neo4jConnection, embedding_provider: EmbeddingProvider
    ):
    ...
    def _generate_entity_id(self, user_id: str, name: str, entity_type: str) -> str:
        """Generate consistent entity ID using deterministic UUID.
        ...
        """
        key = f"{user_id}:{entity_type}:{name.lower().strip()}"
        return str(uuid.uuid5(uuid.NAMESPACE_DNS, key))

    async def search_similar_nodes(
        self, query: str, threshold: float = 0.8, user_id: str | None = None
    ) -> str:
        ...
        try:
            ...
            if not results:
                return (
                    f"No similar nodes found for query '{query}' "
    ...
    async def upsert_node(
        ...
        try:
            ...
            if merge_id and merge_id.strip():
                ...
                # Create new node - require user_id for deterministic ID generation
                if not user_id:
                    raise ValueError("user_id is required when creating new nodes")

                entity_id = self._generate_entity_id(user_id, name, label)
                ...
                if user_id:
                    cypher += "\nSET n.user_id = $user_id"
                    create_parameters["user_id"] = user_id

                # Only set properties if they exist and are not empty
                if properties:
                    cypher += "\nSET n.properties = $properties"
                    create_parameters["properties"] = properties

                cypher += "\nRETURN n.entity_id as id, n.name as name"

                result = await self.neo4j_client.execute_write_query(
                    cypher, create_parameters
                )
    ...
    async def create_relationship(
        ...
        try:
            ...
            parameters: dict[str, Any] = {"from_name": from_name, "to_name": to_name}
            ...
            cypher += (
                "\nRETURN from_node.name as from_name, "
                "to_node.name as to_name, type(r) as rel_type"
            )
            ...
            if result and len(result) > 0:
                return (
                    f"Success: Created relationship '{relation_type}' "
                    f"from '{from_name}' to '{to_name}'"
                )
            else:
                # Try to find which nodes exist
                check_cypher = """
                OPTIONAL MATCH (from_node) 
                WHERE from_node.name = $from_name
                OPTIONAL MATCH (to_node) 
                WHERE to_node.name = $to_name
                RETURN from_node IS NOT NULL as from_exists, 
                       to_node IS NOT NULL as to_exists
                """

                check_result = await self.neo4j_client.execute_query(
                    check_cypher, {"from_name": from_name, "to_name": to_name}
                )
                ...
                if check_result:
                    ...
                    if not from_exists and not to_exists:
                        return (
                            f"Error: Both nodes '{from_name}' and "
                            f"'{to_name}' not found"
                        )
                ...
                return (
                    f"Error: Failed to create relationship between "
    ...
    async def delete_relationship(
        ...
        Args:
            from_name: Source node name
            to_name: Target node name
            relation_type: Relationship type to delete
        ...
        try:
            ...
            if user_id:
                where_clause = (
                    "AND from_node.user_id = $user_id " "AND to_node.user_id = $user_id"
                )

            cypher = f"""
            MATCH (from_node)-[r:{relation_type}]->(to_node)
            WHERE from_node.name = $from_name AND to_node.name = $to_name
            {where_clause}
            DELETE r
            ...
            if result and result[0].get("deleted_count", 0) > 0:
                ...
                return (
                    f"Success: Deleted {deleted_count} '{relation_type}' "
                    f"relationship(s) from '{from_name}' to '{to_name}'"
                )
            else:
                ...
                if check_result:
                    ...
                    if not from_exists and not to_exists:
                        ...
                        return (
                            f"Error: No '{relation_type}' relationship found "
                            f"between '{from_name}' and '{to_name}'"
                        )

                return (
                    f"Error: Failed to delete relationship between "
                    f"'{from_name}' and '{to_name}'"
                )

        except Exception as e:
            logger.error(f"Delete relationship failed: {e}")
            return f"Error: Failed to delete relationship '{relation_type}' - {str(e)}"
