...
class EnhancedContextAssembler:
    ...
    def __init__(self, memory_manager: MemoryManager):
        ...
        self.max_retrieval_latency_ms = 2000  # Increased to allow for embedding generation and vector search
        self.enable_entity_extraction = True
        
        logger.info("🧠 Enhanced Context Assembler initialized with RAG++ memory integration")
    ...
    async def assemble_context(
        ...
        logger.debug(f"🔍 [CTX-{request_id}] Assembling memory-aware context for: {current_input[:50]}...")
        ...
        try:
            ...
            logger.debug(f"🧠 [CTX-{request_id}] Starting parallel memory retrieval from 4 tiers...")
            ...
            logger.info(f"✅ [CTX-{request_id}] Context assembled successfully ({assembly_time:.2f}ms)")
            
            return prompt
            
        except asyncio.TimeoutError:
            logger.warning(f"⏰ [CTX-{request_id}] Context assembly timeout, using fallback")
            return await self._build_fallback_prompt(user_id, current_input, state, available_tools)
    ...
    async def _extract_entities_safe(self, text: str, request_id: str) -> List[str]:
        ...
        try:
            async with self.entity_extractor as extractor:
                ...
                logger.debug(f"📝 [CTX-{request_id}] Extracted {len(entities)} entities: {entities[:5]}")
                return entities
                
        except Exception as e:
            logger.warning(f"⚠️  [CTX-{request_id}] Entity extraction failed, using keyword fallback: {e}")
    ...
    def _extract_keywords_simple(self, text: str) -> List[str]:
        ...
        for word in words:
            ...
            if (
                ...
                (word.istitle() or clean_word in ['email', 'file', 'search', 'weather', 'calendar'])
            ):
    ...
    async def _build_memory_section(self, memory_context: Dict[str, Any]) -> str:
        ...
        if short_term:
            if short_term.get("summary"):
                memory_parts.append(f"RECENT CONVERSATION SUMMARY:\n{short_term['summary']}")
            ...
            if short_term.get("recent_messages"):
                ...
                for msg in recent_messages:  # Use ALL messages in buffer, not just last 5
                    user_msg = msg.get("user_message", "")
                    assistant_msg = msg.get("assistant_response", "")
                    # Clean system info from assistant response for cleaner context
                    clean_assistant_msg = assistant_msg.split("---")[0].strip() if assistant_msg else ""
        ...
        if semantic and semantic.get("entities"):
            ...
            entity_names = [entity.get("name", entity.get("type", "Unknown")) for entity in entities if entity]
            if entity_names:
...
Use tools when they can help accomplish the user's goals. Always explain what you're doing."""
    ...
    def _build_situation_section(
        ...
        if state.awaiting_confirmation:
            situation_parts.append("- Status: Awaiting user confirmation for pending action")
    ...
    def configure_performance(
        ...
        enable_entity_extraction: bool = None
    ) -> None:
        ...
        if max_latency_ms is not None:
            ...
            logger.info(f"⚙️  Context assembler latency budget set to {max_latency_ms}ms")
        
        if enable_entity_extraction is not None:
            self.enable_entity_extraction = enable_entity_extraction
            logger.info(f"⚙️  Entity extraction {'enabled' if enable_entity_extraction else 'disabled'}")


# Factory function for easy integration
async def create_enhanced_context_assembler(memory_manager: MemoryManager = None) -> EnhancedContextAssembler:
